-- This script updates the gifting logic to reset weekly limits
-- every Sunday at 12:00 AM EST (America/New_York timezone).

-- Function 1: Update the main gifting function with the new reset logic.
...
-- Function 2: A new helper function to safely get the current week's gift count.
-- This is used by the frontend to display the correct numbers.
...
DECLARE
    v_lines_gifted_val INT;
    v_nukes_gifted_val INT;
    v_week_start DATE;
    v_current_week_start_est DATE;
BEGIN
    v_current_week_start_est := (date_trunc('week', (now() AT TIME ZONE 'America/New_York' + interval '1 day')) - interval '1 day')::date;
    ...
    SELECT lines_gifted_this_week, nukes_gifted_this_week, week_start_date
    INTO v_lines_gifted_val, v_nukes_gifted_val, v_week_start
    FROM gifting_limits
    ...
    ELSIF v_week_start < v_current_week_start_est THEN
        RETURN QUERY SELECT 0, 0;
    ELSE
        RETURN QUERY SELECT v_lines_gifted_val, v_nukes_gifted_val;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
