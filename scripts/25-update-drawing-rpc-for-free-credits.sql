-- This script refactors the `spend_credit_and_draw` function to prioritize
-- using free, session-locked credits before falling back to paid credits.
...
BEGIN
    ...
    -- 2. Check for and use free credits first
    SELECT amount INTO v_free_credits
    FROM free_line_credits
    WHERE user_wallet_address = p_drawer_wallet_address
      AND session_owner_wallet_address = v_streamer_wallet_address
    FOR UPDATE;

    IF v_free_credits IS NOT NULL AND v_free_credits > 0 THEN
        UPDATE free_line_credits
        SET amount = amount - 1
        WHERE user_wallet_address = p_drawer_wallet_address
          AND session_owner_wallet_address = v_streamer_wallet_address;
