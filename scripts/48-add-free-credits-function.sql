-- Drop the function if it exists to ensure a clean setup, specifying arguments for precision.
DROP FUNCTION IF EXISTS get_user_free_credit_sessions(TEXT);

-- Recreate the function with the correct return type and robust logic.
...
BEGIN
    ...
    SELECT
        ...
        -- Get the most recent grant time between lines and nukes for sorting purposes.
        -- Defaults to a very old date if one or both are NULL to avoid errors.
    ...
    -- The crucial WHERE clause: only return rows where the user actually has free credits.
    WHERE
