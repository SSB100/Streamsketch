...
-- It correctly calculates the SOL amount from the stored USD amount and SOL price.
...
BEGIN
    -- 1. Calculate the total volume in SOL by dividing the USD amount by the SOL price at the time of transaction.
    -- We add a check to prevent division by zero or null.
    SELECT COALESCE(SUM(amount_usd / sol_price_usd), 0)
